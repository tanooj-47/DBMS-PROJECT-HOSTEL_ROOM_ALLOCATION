...
-- Query 5: List unallocated students
SELECT student_name FROM Students
WHERE student_id NOT IN (SELECT student_id FROM Room_Allocation);

-- Query 6: Find rooms with remaining capacity
...
-- Query 9: Find hostels with more than 1 student allocated
...
-- Query 10: Show students allocated in alphabetical order
SELECT 
    s.student_name
FROM
    Students s
...
-- Query 11: Display hostel capacity vs allocated students
...
-- Query 12: List students allocated after a specific date
SELECT s.student_name, ra.allocation_date FROM Students s
JOIN Room_Allocation ra ON s.student_id = ra.student_id
WHERE ra.allocation_date > '2025-01-10';

-- Query 13: Find students sharing the same room
